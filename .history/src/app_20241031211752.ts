...
interface Usuario {
  ...
  nome: string;
  email: string;
  senha: string; // Senha pode ser omitida ao retornar dados do usuário
}
...
app.post('/api/login', (req: Request, res: Response): Promise<Response> => {
  const { email, senha } = req.body;

  try {
    const result = await client.query<Usuario>('SELECT * FROM usuarios WHERE email = $1', [email]);
